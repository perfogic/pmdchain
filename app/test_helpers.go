...
func Setup(isCheckTx bool, isSetBalance bool, patchGenesis func(*App, simapp.GenesisState) simapp.GenesisState) *App {
	return SetupWithDB(isCheckTx, isSetBalance, patchGenesis, dbm.NewMemDB())
}

// SetupWithDB initializes a new App. A Nop logger is set in App.
func SetupWithDB(isCheckTx bool, isSetBalance bool, patchGenesis func(*App, simapp.GenesisState) simapp.GenesisState, db dbm.DB) *App {
	...
	if !isCheckTx {
		...
		if isSetBalance == true {
			genesisState = NewTestGenesisStateWithBalances(app.AppCodec())
		}
		if isSetBalance == false {
...
func NewTestGenesisStateWithBalances(codec codec.Codec) simapp.GenesisState {
	privVal := mock.NewPV()
